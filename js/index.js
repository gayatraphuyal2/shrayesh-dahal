/**
 * function to show Solar Nepal Sambat Calendar and other helper funtions (part of Brihat Calendar)
...
function add_author_info(month, year) {
  ...
  let copyright_notice = "&copy;<br /><b>Bhupendra Dahal</b><br />";
...
function fill_lunar_data(year1, year2) {
    ...
    const lunar_data_url1 = "https://gayatraphuyal2.github.io/shrayesh-dahal/data/data.json";
    ...
    if (year1 >= 2070 && year1 <= END_BS_YEAR) {
      ...
      lunar_data_req1.open('GET', lunar_data_url1, true);
...
//     }
    ...
    table_headers.innerHTML += "<th>सोम</th><th>मङ्ल</th>";
    table_headers.innerHTML += "<th>बुधब</th><th>बिहि</th><th>सुक्र</th>";
    table_headers.innerHTML += "<th class='saturday'>सनि</th>";
    ...
    monthAndYear.innerHTML += "<div id='footer'>Bhupendra Dahal</div>"
    ...
    main_title.innerText = NS_NEP[month - 1] + " " + arabic_number_to_nepali(year) + " (Calendar)";
    ...
    show_public1_holidays(bs_year_start, bs_year_end, bs_month_start, bs_month_end);
    main_title.innerText = NS_NEP[month - 1] + " " + arabic_number_to_nepali(year) + " (Calendar)";
    add_author_info(NS_NEP[month - 1], arabic_number_to_nepali(year));
    add_muhoortta_box(month, year);
    add_parvas_list_ns(month, year);
    ...
    show_public2_holidays(bs_year_start, bs_year_end, bs_month_start, bs_month_end);
    main_title.innerText = NS_NEP[month - 1] + " " + arabic_number_to_nepali(year) + " (Calendar)";
    add_author_info(NS_NEP[month - 1], arabic_number_to_nepali(year));
    add_muhoortta_box(month, year);
    add_parvas_list_ns(month, year);
    ...
    show_public3_holidays(bs_year_start, bs_year_end, bs_month_start, bs_month_end);
    main_title.innerText = NS_NEP[month - 1] + " " + arabic_number_to_nepali(year) + " (Calendar)";
    add_author_info(NS_NEP[month - 1], arabic_number_to_nepali(year));
    add_muhoortta_box(month, year);
    add_parvas_list_ns(month, year);
...
console.info("Shrayesh Calendar  Copyright (C) 2020-", AD_TODAY_YEAR,"  Bhupendra Dahal");
console.info("* * *");
