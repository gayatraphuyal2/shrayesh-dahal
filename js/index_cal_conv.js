/**
 * functions to show AD and BS Calendar (part of Brihat Calendar)
...
//     }
    ...
    monthAndYear.innerHTML += "<div id='footer'>Bhupendra Dahal";
    ...
    main_title.innerText = AD_MONTHS[month - 1] + " " + year + " (Shrayesh Calendar)";
    ...
    show_public1_holidays(bs_year_start, bs_year_end, bs_start_date_list_from_ns[1], bs_end_date_list_from_ns[1]);
    main_title.innerText = AD_MONTHS[month - 1] + " " + year + " (Shrayesh Calendar)";
    add_author_info(AD_MONTHS[month - 1], year);
    add_muhoortta_box(month, year);
    add_parvas_list_ad(month, year);
    add_public1_holiday_box(month, year);
    update_date_jumper(CALENDAR_MODE);
    console.info("Generating Anno Domini Calendar... DONE!");

    show_public2_holidays(bs_year_start, bs_year_end, bs_start_date_list_from_ns[1], bs_end_date_list_from_ns[1]);
    main_title.innerText = AD_MONTHS[month - 1] + " " + year + " (Shrayesh Calendar)";
    add_author_info(AD_MONTHS[month - 1], year);
    add_muhoortta_box(month, year);
    add_parvas_list_ad(month, year);
    add_public2_holiday_box(month, year);
    update_date_jumper(CALENDAR_MODE);
    console.info("Generating Anno Domini Calendar... DONE!");

    show_public3_holidays(bs_year_start, bs_year_end, bs_start_date_list_from_ns[1], bs_end_date_list_from_ns[1]);
    main_title.innerText = AD_MONTHS[month - 1] + " " + year + " (Shrayesh Calendar)";
    add_author_info(AD_MONTHS[month - 1], year);
    add_muhoortta_box(month, year);
    add_parvas_list_ad(month, year);
    add_public3_holiday_box(month, year);
    update_date_jumper(CALENDAR_MODE);
    console.info("Generating Anno Domini Calendar... DONE!");
}
...
function showBSCalendar(month, year) {
    ...
    if (year == 2079 && month == 2) {
      table_headers.innerHTML = "<th class='sundaytrial'>आइत</th>";
    }
    else {
      table_headers.innerHTML = "<th>आइत</th>";
    }
    table_headers.innerHTML += "<th>सोम</th><th>मङ्गल</th>";
    table_headers.innerHTML += "<th>बुध</th><th>बिहि</th><th>शुक्र</th>"
    table_headers.innerHTML += "<th class='saturday'>शनि</th>";
    ...
    monthAndYear.innerHTML += "<h6><span style='color: green ='showADCalendar(BS_TODAY_MONTH + 1, BS_TODAY_YEAR)'>" + ad_month_year + "</span></h6>";
    monthAndYear.innerHTML += "<div id='lunar_details'></div>";
    monthAndYear.innerHTML += "<div id='footer'>Develop by Bhupendra Dahal</div>";
    // update Go To section as well
    select_year.value = ns_year_start;
    select_month.value = ns_month_start;
    ...
    main_title.innerText = BS_MONTHS_NEP[month - 1] + " " + arabic_number_to_nepali(year) + " (Shrayesh Calendar)";
    ...
    show_public1_holidays(year, year, month, month);
    main_title.innerText = BS_MONTHS_NEP[month - 1] + " " + arabic_number_to_nepali(year) + " (Shrayesh Calendar)";
    add_author_info(BS_MONTHS_NEP[month - 1], arabic_number_to_nepali(year));
    add_muhoortta_box(month, year);
    add_parvas_list_bs(month, year);
    document.getElementById("public1_holiday_box").style.display = "none";
    update_date_jumper(CALENDAR_MODE);
    console.info("Generating Bikram Sambat Calendar... DONE!");

    show_public2_holidays(year, year, month, month);
    main_title.innerText = BS_MONTHS_NEP[month - 1] + " " + arabic_number_to_nepali(year) + " (Shrayesh Calendar)";
    add_author_info(BS_MONTHS_NEP[month - 1], arabic_number_to_nepali(year));
    add_muhoortta_box(month, year);
    add_parvas_list_bs(month, year);
    document.getElementById("public2_holiday_box").style.display = "none";
    update_date_jumper(CALENDAR_MODE);
    console.info("Generating Bikram Sambat Calendar... DONE!");

    show_public3_holidays(year, year, month, month);
    main_title.innerText = BS_MONTHS_NEP[month - 1] + " " + arabic_number_to_nepali(year) + " (Shrayesh Calendar)";
    add_author_info(BS_MONTHS_NEP[month - 1], arabic_number_to_nepali(year));
    add_muhoortta_box(month, year);
    add_parvas_list_bs(month, year);
    document.getElementById("public3_holiday_box").style.display = "none";
    update_date_jumper(CALENDAR_MODE);
    console.info("Generating Bikram Sambat Calendar... DONE!");
}
