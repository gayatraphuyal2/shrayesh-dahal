...
function create_table(year = "") {
    ...
    if(year != "") {
        var lunar_event_url_2 = "https://bhupendra12345678.github.io/shrayesh-dahal/data/" + year.toString() + "_detailed.json";
        var lunar_event_req_2 = new XMLHttpRequest();
        lunar_event_req_2.open('GET', lunar_event_url_2, true);
    ...
    for(var i = 0; i < lunar_data.length; i++) {
        for(var j = 0; j < lunar_data[i].length; j++) {
            ...
            // BS AD NS DATE
            var table_data_date = arabic_number_to_nepali(bs_date_split[0]) + " " + BS_MONTHS_NEP[bs_date_split[1] - 1] + " " + arabic_number_to_nepali(bs_date_split[2].replace(/^0+/, '')) + " -" + NEPALI_DAYS_SHORT[weekday] + "बार<br />";
            table_data_date += (date_data.pakshya)  + date_data.lunar_month + "&nbsp;" + date_data.tithi + "<br />";
            table_data_date + ")";
...
function change_list(year = "") {
    ...
    event_dt = $('#event_table').DataTable({
        ...
        "drawCallback": function( settings ) {
            ...
            api.column(event_start_index++).visible(true);
            api.column(event_start_index++).visible(true);
            api.column(event_start_index++).visible(false);
    ...
    if(year == today_bs_date[0]) {
        ...
        today_element.classList.add("text-primary");
        today_element.classList.add("text-decoration-underline");
